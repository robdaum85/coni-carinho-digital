  ...
  return <section id="depoimentos" className="py-20 bg-white">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="relative mb-16">
          {/* Navigation Buttons */}
          <button onClick={prevSlide} className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-12 h-12 bg-white rounded-full shadow-lg flex items-center justify-center text-modern-primary hover:bg-modern-primary hover:text-white transition-all duration-300 hover-scale" aria-label="Previous testimonial">
            <ChevronLeft size={20} />
          </button>
          
          <button onClick={nextSlide} className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-12 h-12 bg-white rounded-full shadow-lg flex items-center justify-center text-modern-primary hover:bg-modern-primary hover:text-white transition-all duration-300 hover-scale" aria-label="Next testimonial">
          ...
          <div className="mx-12">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {getVisibleTestimonials().map((testimonial, index) => <div key={index} className="testimonial-card animate-fade-in-up">
          ...
          }).map((_, index) => <button key={index} onClick={() => setCurrentSlide(index * slidesToShow)} className={`w-3 h-3 rounded-full transition-all duration-300 ${Math.floor(currentSlide / slidesToShow) === index ? 'bg-modern-primary' : 'bg-modern-border hover:bg-modern-primary/50'}`} aria-label={`Go to slide ${index + 1}`} />)}
          </div>
        </div>
        ...
        <div className="flex justify-center animate-fade-in-up">
          <div className="rounded-2xl overflow-hidden shadow-xl w-full max-w-lg hover-lift">
            <img alt="Mãe feliz com seu bebê" className="w-full h-auto object-cover" loading="lazy" src="/lovable-uploads/84414bf5-5064-4600-843b-408056150760.png" />
          </div>
        </div>
      </div>
    </section>;
